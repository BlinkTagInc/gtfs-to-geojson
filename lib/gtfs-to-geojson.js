  ...
  if (config.outputType === 'route') {
    ...
    await Promise.all(routes.map(async route => {
      ...
      await Promise.all(_.map(directionGroups, async directionGroup => {
        ...
        const geojson = await fetchGeoJSON(agencyKey, route.route_id, direction.direction_id, config);
        const simplifiedGeoJSON = utils.simplifyGeoJSON(geojson, config.coordinatePrecision);
        stats.files.value += 1;
        const fileName = `${utils.getRouteName(route)}_${direction.direction_id}.geojson`;
        await utils.writeFile(utils.getExportPath(agencyKey), fileName, JSON.stringify(simplifiedGeoJSON));
      }));
    }));
  } else if (config.outputType === 'agency') {
    const geojson = await fetchGeoJSON(agencyKey, undefined, undefined, config);
    const simplifiedGeoJSON = utils.simplifyGeoJSON(geojson, config.coordinatePrecision);
    stats.files.value += 1;
