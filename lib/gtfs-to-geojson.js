...
module.exports = selectedConfig => {
  ...
  return Promise.all(config.agencies.map(agency => {
    ...
    .then(routes => {
      if (config.outputType === 'direction') {
        return Promise.all(routes.map(route => {
          ...
          .then(directions => {
            ...
            return Promise.all(_.map(directionGroups, directionGroup => {
              ...
              .then(geojson => {
                const fileName = sanitize(`${utils.getRouteName(route)}_${direction.direction_id}.geojson`);
                return fs.writeFile(path.join(exportPath, fileName), JSON.stringify(geojson));
              });
            }));
          });
        }));
        ...
        .then(geojson => {
          ...
          const fileName = sanitize(`${agencyKey}.geojson`);
          return fs.writeFile(path.join(exportPath, fileName), JSON.stringify(geojson));
        });
      }
    ...
    .then(logText => fs.writeFile(path.join(exportPath, 'log.txt'), logText.join('\n')))
    .then(() => {
